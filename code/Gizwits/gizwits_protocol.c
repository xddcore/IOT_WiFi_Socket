...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagTime_OnOff)
    {
        ...
        info->num++;
        dataPoints->valueTime_OnOff = gizDecompressionValue(TIME_ONOFF_BYTEOFFSET,TIME_ONOFF_BITOFFSET,TIME_ONOFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagCountDown_OnOff)
    {
        info->event[info->num] = EVENT_COUNTDOWN_ONOFF;
        info->num++;
        dataPoints->valueCountDown_OnOff = gizDecompressionValue(COUNTDOWN_ONOFF_BYTEOFFSET,COUNTDOWN_ONOFF_BITOFFSET,COUNTDOWN_ONOFF_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagWeek_Repeat)
    {
        info->event[info->num] = EVENT_WEEK_REPEAT;
        info->num++;
        dataPoints->valueWeek_Repeat = gizX2Y(WEEK_REPEAT_RATIO,  WEEK_REPEAT_ADDITION, issuedData->attrVals.valueWeek_Repeat); 
    }



    if(0x01 == issuedData->attrFlags.flagTime_On_Minute)
    {
        info->event[info->num] = EVENT_TIME_ON_MINUTE;
        info->num++;
        dataPoints->valueTime_On_Minute = gizX2Y(TIME_ON_MINUTE_RATIO,  TIME_ON_MINUTE_ADDITION, gizProtocolExchangeBytes(issuedData->attrVals.valueTime_On_Minute));
    }



    if(0x01 == issuedData->attrFlags.flagTime_Off_Minute)
    {
        info->event[info->num] = EVENT_TIME_OFF_MINUTE;
        info->num++;
        dataPoints->valueTime_Off_Minute = gizX2Y(TIME_OFF_MINUTE_RATIO,  TIME_OFF_MINUTE_ADDITION, gizProtocolExchangeBytes(issuedData->attrVals.valueTime_Off_Minute));
    }



    if(0x01 == issuedData->attrFlags.flagCountDown_Minute)
    {
        info->event[info->num] = EVENT_COUNTDOWN_MINUTE;
        info->num++;
        dataPoints->valueCountDown_Minute = gizX2Y(COUNTDOWN_MINUTE_RATIO,  COUNTDOWN_MINUTE_ADDITION, gizProtocolExchangeBytes(issuedData->attrVals.valueCountDown_Minute));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueCountDown_OnOff != cur->valueCountDown_OnOff)
    {
        GIZWITS_LOG("valueCountDown_OnOff Changed\n");
        ret = 1;
    }
    if(last->valueWeek_Repeat != cur->valueWeek_Repeat)
    {
        GIZWITS_LOG("valueWeek_Repeat Changed\n");
        ret = 1;
    }
    if(last->valueTime_On_Minute != cur->valueTime_On_Minute)
    {
        GIZWITS_LOG("valueTime_On_Minute Changed\n");
        ret = 1;
    }
    if(last->valueTime_Off_Minute != cur->valueTime_Off_Minute)
    {
        GIZWITS_LOG("valueTime_Off_Minute Changed\n");
        ret = 1;
    }
    if(last->valueCountDown_Minute != cur->valueCountDown_Minute)
    {
        GIZWITS_LOG("valueCountDown_Minute Changed\n");
        ret = 1;
    }

    if(last->valuePower_Consumption != cur->valuePower_Consumption)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuePower_Consumption Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueelectric != cur->valueelectric)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueelectric Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueFuse_failure != cur->valueFuse_failure)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueFuse_failure Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(TIME_ONOFF_BYTEOFFSET,TIME_ONOFF_BITOFFSET,TIME_ONOFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueTime_OnOff);
    gizCompressValue(COUNTDOWN_ONOFF_BYTEOFFSET,COUNTDOWN_ONOFF_BITOFFSET,COUNTDOWN_ONOFF_LEN,(uint8_t *)devStatusPtr,dataPoints->valueCountDown_OnOff);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueWeek_Repeat = gizY2X(WEEK_REPEAT_RATIO,  WEEK_REPEAT_ADDITION, dataPoints->valueWeek_Repeat); 

    devStatusPtr->valueTime_On_Minute = gizProtocolExchangeBytes(gizY2X(TIME_ON_MINUTE_RATIO,  TIME_ON_MINUTE_ADDITION, dataPoints->valueTime_On_Minute)); 
    devStatusPtr->valueTime_Off_Minute = gizProtocolExchangeBytes(gizY2X(TIME_OFF_MINUTE_RATIO,  TIME_OFF_MINUTE_ADDITION, dataPoints->valueTime_Off_Minute)); 
    devStatusPtr->valueCountDown_Minute = gizProtocolExchangeBytes(gizY2X(COUNTDOWN_MINUTE_RATIO,  COUNTDOWN_MINUTE_ADDITION, dataPoints->valueCountDown_Minute)); 
    devStatusPtr->valuePower_Consumption = gizProtocolExchangeBytes(gizY2X(POWER_CONSUMPTION_RATIO,  POWER_CONSUMPTION_ADDITION, dataPoints->valuePower_Consumption)); 
    devStatusPtr->valueelectric = gizProtocolExchangeBytes(gizY2XFloat(ELECTRIC_RATIO,  ELECTRIC_ADDITION, dataPoints->valueelectric));     
    devStatusPtr->valueFuse_failure = gizProtocolExchangeBytes(gizY2XFloat(FUSE_FAILURE_RATIO,  FUSE_FAILURE_ADDITION, dataPoints->valueFuse_failure));     
